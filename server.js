...
app.post('/api/tributaria/analisar', async (req, res) => {
  try {
    const { pdfBase64, texto, mes } = req.body;
    if ((!texto || !texto.trim()) && !pdfBase64) {
      return res.status(400).json({ ok:false, erro:'anexa o PDF ou cola o texto do extrato' });
    }
...
{"data":"AAAA-MM-DD","descricao":"string curta com o nome completo se houver","valor":0.00,"tipo":"entrada|saida","categoria":"uma das da lista acima","naturezaFiscal":"só se a tabela de referência tiver essa informação pro nome, senão string vazia","provavelmentePessoal":false,"confianca":"alta|media|baixa","observacao":""}`;
    ...
    transacoes.forEach((t, i) => { t._tempId = mes + '_' + i; });
...
// SUBSTITUI o mês inteiro se já existia (reprocessar um mês corrige o anterior
// por completo, não empilha duplicado).
app.post('/api/tributaria/salvar', async (req, res) => {
  try {
    const { mes, transacoes } = req.body;
    if (!mes || !Array.isArray(transacoes)) {
      return res.status(400).json({ ok:false, erro:'mes e transacoes (array) são obrigatórios' });
    }
    const db = await lerDB();
    if (!db.analiseTributaria) db.analiseTributaria = {};
    db.analiseTributaria[mes] = {
...
// GET /api/tributaria/:mes — recupera um mês já salvo (pra conferir depois,
// ou pra Fase 4 exportar em PDF/Excel).
app.get('/api/tributaria/:mes', async (req, res) => {
  try {
    ...
    res.json({ ok:true, mes: req.params.mes, ...dadosMes });
  } catch(e) {
    res.status(500).json({ ok:false, erro: e.message });
  }
});

// GET /api/tributaria — lista todos os meses já salvos, com totais rápidos
// (pra Fase 4 mostrar "quais meses já estão prontos" sem carregar tudo).
app.get('/api/tributaria', async (req, res) => {
  try {
    const db = await lerDB();
    const meses = Object.entries(db.analiseTributaria || {}).map(([mes, dados]) => ({
      mes, total: (dados.transacoes||[]).length, salvoEm: dados.salvoEm
