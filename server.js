...
app.get('/api/whatsapp/conversas', async (req, res) => {
  ...
  for (const m of msgs) {
    ...
    if (!porContato[chave] || m.timestamp > porContato[chave].timestamp) {
      porContato[chave] = m;
    }
  }
  const lista = Object.values(porContato)
    .sort((a,b) => (b.timestamp||0) - (a.timestamp||0))
    .map(m => ({
      instancia: m.instancia, telefone: m.telefone, nomeContato: m.nomeContato,
