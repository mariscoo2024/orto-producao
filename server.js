...
{"data":"AAAA-MM-DD","tipo":"entrada|saida","descricao":"string curta com o nome completo se houver","valor":0.00,"categoria":"uma das da lista acima","naturezaFiscal":"só se a tabela de referência tiver essa informação pro nome, senão string vazia","provavelmentePessoal":false,"confianca":"alta|media|baixa","observacao":""}`;
    ...
    if (dataIA.stop_reason === 'max_tokens') {
      ...
      return res.status(500).json({ ok:false, erro: 'Esse mês tem transações demais pra analisar de uma vez. Tenta dividir em duas quinzenas.' });
    }
    ...
    try {
      ...
      try {
        ...
        return res.status(500).json({ ok:false, erro:'A IA não conseguiu estruturar o extrato. Tenta de novo, ou em pedaços menores (ex: quinzena).' });
      }
    }
    if (!Array.isArray(transacoes)) {
      return res.status(500).json({ ok:false, erro:'A IA devolveu um formato inesperado. Tenta de novo, ou em pedaços menores.' });
    }

    // Dá um id temporário pra cada linha, só pra facilitar edição na tela antes de salvar
    transacoes.forEach((t, i) => { t._tempId = mes + '_' + i; t.banco = banco || ''; });

    res.json({ ok:true, mes, transacoes });
  } catch(e) {
    console.error('❌ Erro em /api/tributaria/analisar:', e.message);
    res.status(500).json({ ok:false, erro: e.message });
  }
});
...
// SUBSTITUI só a fonte (banco) daquele mês se já existia — reprocessar
// corrige só aquele banco, não mexe no outro.
app.post('/api/tributaria/salvar', async (req, res) => {
  try {
    const { mes, banco, transacoes } = req.body;
    ...
    db.analiseTributaria[mes][banco] = {
    ...
    const ok = await salvarDB(db);
    res.json({ ok, totalSalvo: transacoes.length });
  } catch(e) {
