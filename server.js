...
app.get('/api/bling/sync', async (req, res) => {
  ...
  try {
    const data = await httpsGet('www.bling.com.br',
      '/Api/v3/pedidos/vendas?limite=100&pagina=1&idsSituacoes[]=6&idsSituacoes[]=9&idsSituacoes[]=15&idsSituacoes[]=12&idsSituacoes[]=24',
      { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' });
    ...
    for (const pedBling of data.data) {
      ...
      db.pedidos.push(parsearPedidoBling(pedData));
      novos++;
