...
// Railway fica atrás de um proxy/load balancer — sem isso, req.ip sempre
// devolve o IP interno do proxy, não o IP real de quem acessou. Precisa disso
// pra função de "bater ponto só com IP da fábrica" funcionar de verdade.
app.set('trust proxy', true);

// ── Ponto eletrônico: IPs autorizados a bater ponto ─────────────────────────
// Lista separada por vírgula na variável FABRICA_IPS do Railway (ex: "189.45.12.30").
// Sem essa variável configurada, NINGUÉM consegue bater ponto (trava aberta é
// pior que trava fechada aqui — evita que rode "sem querer" sem checar IP).
// Pra descobrir o IP certo: abra o app conectado no Wi-Fi da fábrica e acesse
// GET /api/ponto/meu-ip — ele mostra o IP que está sendo visto agora.
const FABRICA_IPS = (process.env.FABRICA_IPS || '').split(',').map(s => s.trim()).filter(Boolean);
if (FABRICA_IPS.length === 0) {
  console.warn('⚠️ FABRICA_IPS não configurado nas variáveis do Railway — bater ponto vai ficar bloqueado pra todo mundo até configurar.');
}
function ipCliente(req) {
  let ip = req.ip || (req.connection && req.connection.remoteAddress) || '';
  if (ip.startsWith('::ffff:')) ip = ip.slice(7); // IPv4 mapeado em IPv6, formato do Node
  return ip;
}
function ipAutorizado(ip) {
  return FABRICA_IPS.length > 0 && FABRICA_IPS.includes(ip);
}
// Horário de Brasília calculado explicitamente — o servidor do Railway roda em
// UTC por padrão. Sem isso, ponto batido às 21h apareceria como 00h (ou até no
// dia seguinte, perto da meia-noite) — bug grave num sistema de ponto.
function agoraBrasil() {
  ...
  return { data: `${p.year}-${p.month}-${p.day}`, horario: `${p.hour}:${p.minute}:${p.second}` };
}
...
// ── Evolution API (WhatsApp) — piloto de monitoramento de conversas dos vendedores.
// EVOLUTION_API_URL/EVOLUTION_API_KEY só são usados se um dia o sistema precisar
// CHAMAR a Evolution API (ex: criar instância por aqui). Por enquanto, o piloto só
// RECEBE mensagens via webhook, então só EVOLUTION_WEBHOOK_SECRET é obrigatório.
const EVOLUTION_API_URL      = process.env.EVOLUTION_API_URL || '';
const EVOLUTION_API_KEY      = process.env.EVOLUTION_API_KEY || '';
const EVOLUTION_WEBHOOK_SECRET = process.env.EVOLUTION_WEBHOOK_SECRET || '';
if (!EVOLUTION_WEBHOOK_SECRET) {
  console.warn('⚠️ EVOLUTION_WEBHOOK_SECRET não configurado — o webhook /webhook/evolution vai recusar tudo até essa variável existir no Railway.');
}

// Transcrição de áudio do WhatsApp — só usada quando alguém clica "Transcrever"
// num áudio específico (sob demanda, não automático, pra não gastar sem necessidade).
...
const SEED_CONCILIACAO_BANCO = [{"nomeBanco": "SH ARTIGOS PARA ESTOFADOS", "categoria": "materia_prima", "banco": "Ambos", "obs": "Fornecedor de estofamento/tecido — maior fornecedor do mês", "id": "conc_seed_0"}, {"nomeBanco": "ARW ARTEFATOS DE MADEIRA", "categoria": "materia_prima", "banco": "Ambos", "obs": "Fornecedor de madeira/base", "id": "conc_seed_1"}, {"nomeBanco": "ALEIXO ANTONIO STABACH", "categoria": "materia_prima", "banco": "Ambos", "obs": "", "id": "conc_seed_2"}, {"nomeBanco": "SOBRAL COMERCIO LTDA", "categoria": "materia_prima", "banco": "Ambos", "obs": "Aparece só quando tem compra (Mario/Sobral)", "id": "conc_seed_3"}, {"nomeBanco": "DONDOKO TECIDOS", "categoria": "materia_prima", "banco": "PagBank", "obs": "Aparece só quando tem compra", "id": "conc_seed_4"}, {"nomeBanco": "JPL SUBLIMACAO E TECIDOS LTDA", "categoria": "materia_prima", "banco": "PagBank", "obs": "Etiquetas/logo (JPL = as logo)", "id": "conc_seed_5"}, {"nomeBanco": "DECOR MAXI", "categoria": "materia_prima", "banco": "Ambos", "obs": "Fornecedor", "id": "conc_seed_6"}, {"nomeBanco": "ROHI MOVEIS", "categoria": "ads_marketing", "banco": "Ambos", "obs": "CONFIRMADO jul/2026: bate quase exato com o ADS do DRE (R$39.590 pago x R$39.608 reconhecido)", "id": "conc_seed_7"}, {"nomeBanco": "LUIZ CARLOS DA SILVA", "categoria": "aluguel_fixo", "banco": "Santander", "obs": "CONFIRMADO jul/2026: aluguel dos 2 barracões, bate exato (R$5.950+R$2.000=R$7.950)", "id": "conc_seed_8"}, {"nomeBanco": "WESLEY KLEBER DOS SANTOS", "categoria": "materia_prima", "banco": "Santander", "obs": "NÃO é o Wesley dono — é o Chaperini/costureiro, ganha R$50/colchão fixo. Já embutido na tabela Custo por modelo — não duplicar em Folha do DRE.", "naturezaFiscal": "servico_pf_recorrente", "obsFiscal": "Pessoa física, ~R$10mil/mês, pagamento recorrente e habitual por serviço. Para a análise tributária NÃO é matéria-prima — é mão de obra terceirizada PF. Levar pra consultoria avaliar risco de retenção de INSS e caracterização de vínculo (confirmado com Wesley em 16/08/2026).", "id": "conc_seed_9"}, {"nomeBanco": "VERA LUCIA DE FATIMA DE SOUZA", "categoria": "prolabore_pessoal", "banco": "PagBank", "obs": "É a mãe do Wesley — usada pra retirada de pró-labore por bloqueio no CPF dele", "id": "conc_seed_10"}, {"nomeBanco": "MATHEUS SCARPAT DA SILVA", "categoria": "prolabore_pessoal", "banco": "Santander", "obs": "Aluguel pessoal do Wesley, tratado como pró-labore", "id": "conc_seed_11"}, {"nomeBanco": "OP MOVEIS PREMIUM", "categoria": "prolabore_pessoal", "banco": "Santander", "obs": "Relacionado à mãe do Wesley — tratar como pessoal", "id": "conc_seed_12"}, {"nomeBanco": "ALAN ROSA RIBEIRO", "categoria": "repasse_nao_receita", "banco": "Ambos", "obs": "Passou dinheiro na maquininha do Wesley por emergência — ele pagou de volta 1:1, taxa zero pra Orto Premium. NÃO é venda real nem despesa real, é passagem.", "id": "conc_seed_13"}, {"nomeBanco": "Pagamento de empréstimo - Recuperação do Empréstimo", "categoria": "emprestimo", "banco": "PagBank", "obs": "Descontado picado por venda. Confirmar taxa de juros efetiva — pode estar cara.", "id": "conc_seed_14"}, {"nomeBanco": "MAX BORDADO", "categoria": "materia_prima", "banco": "", "obs": "Aparece só quando tem compra (ainda não visto em extrato)", "id": "conc_seed_15"}, {"nomeBanco": "MARIO", "categoria": "materia_prima", "banco": "", "obs": "Aparece só quando tem compra (ainda não visto em extrato)", "id": "conc_seed_16"}, {"nomeBanco": "GUIMA TRANSPORTES LTDA", "categoria": "frete", "banco": "Ambos", "obs": "CONFIRMADO ago/2026: freteiro. Frete já é calculado automaticamente por pedido no sistema de cotação — NUNCA lançar como avulso no DRE (duplicaria o custo).", "id": "conc_seed_17"}, {"nomeBanco": "JOEL ANDRE RIBEIRO", "categoria": "frete", "banco": "Ambos", "obs": "CONFIRMADO ago/2026: freteiro — mesma regra, não lançar como avulso.", "id": "conc_seed_18"}, {"nomeBanco": "JONATHAN ALCIDES DE SIQUEIRA", "categoria": "frete", "banco": "Ambos", "obs": "CONFIRMADO ago/2026: freteiro (Jhonathan/Siqueira) — não lançar como avulso.", "id": "conc_seed_19"}, {"nomeBanco": "VALMIR TOME DOS SANTOS", "categoria": "frete", "banco": "Ambos", "obs": "CONFIRMADO ago/2026: freteiro (Tomé) — não lançar como avulso.", "id": "conc_seed_20"}, {"nomeBanco": "JOAO HERMES", "categoria": "emprestimo", "banco": "Santander", "obs": "PIX de R$1.500 redondo — parcela do financiamento da Montana. Mesma regra do Vera PJ R$1.500 (confirmado com Wesley em 16/08/2026).", "id": "conc_seed_21"}, {"nomeBanco": "CHRISTIAN DE OLIVEIRA", "categoria": "folha", "banco": "Santander", "obs": "Funcionário — apareceu no teste da análise tributária de jan/2026 (16/08/2026).", "id": "conc_seed_22"}, {"nomeBanco": "LUIZ FERNANDO VITORIO SOARES CORDEIRO", "categoria": "folha", "banco": "Santander", "obs": "Funcionário de folha — NÃO confundir com LUIZ CARLOS DA SILVA (esse é o aluguel do barracão, pessoa diferente). Nome completo confirmado no teste de jan/2026 (16/08/2026).", "id": "conc_seed_23"}, {"nomeBanco": "AYMORE CREDITO", "categoria": "emprestimo", "banco": "Santander", "obs": "Financeira (Aymoré/Santander) — provável parcela de financiamento de veículo ou equipamento. Confirmar com Wesley o que está sendo financiado. Apareceu no teste de jan/2026 (16/08/2026).", "id": "conc_seed_24"}];

function dbVazio() {
  return { pedidos:[], rastreios:[], estoque:[], sobras:[], movHist:{}, custos:{}, blingToken:{}, tabelaPrecos:{}, custosModelo:{}, custosFixos:{}, cotacoesFrete:[], cfgFrete:null, pedidosIgnorados:[], cidadesFreteiro:[], freteirosCfg:{}, catalogoSku:[], pontos:[], logoBase64:null, conciliacaoBanco:[], analiseTributaria:{} };
}
...
// ── Sincronização de status com a planilha do Google (Apps Script) ──────────
// URLs de Web App do Apps Script às vezes respondem com redirect (301/302)
// antes do conteúdo de verdade — POST simples sem seguir redirect falharia
// silenciosamente. Essa versão segue até 3 redirects.
function httpsPostSeguindoRedirect(urlStr, body, maxRedirects=3) {
  return new Promise((resolve, reject) => {
    const payload = typeof body === 'string' ? body : JSON.stringify(body);
    let u;
    ...
    const req = https.request(options, res => {
      if ([301,302,303,307,308].includes(res.statusCode) && res.headers.location && maxRedirects > 0) {
        res.resume();
        httpsPostSeguindoRedirect(res.headers.location, body, maxRedirects-1).then(resolve, reject);
        return;
      }
      let data = '';
      res.on('data', d => data += d);
...
// Nasceu do pedido: "quando bipar Embalagem/Embarque, mudar sozinho o
// status na planilha do Google (coluna O), achando o cliente pelo nome".
// A planilha continua sendo uma segunda fonte de dado (risco já registrado
// e aceito conscientemente) — isso aqui só evita ter que atualizar os dois
// lugares na mão toda vez.
const SHEET_WEBHOOK_URL = process.env.SHEET_WEBHOOK_URL || '';
if (!SHEET_WEBHOOK_URL) {
  console.warn('⚠️ SHEET_WEBHOOK_URL não configurado — sincronização com a planilha do Google está desligada até configurar essa variável no Railway.');
}
...
async function refreshBlingToken() {
  ...
  try {
    ...
    const data = await httpsPost('api.bling.com.br', '/Api/v3/oauth/token',
...
// ── Notificar planilha do Google (Apps Script) quando bipar Embalagem/Embarque
app.post('/api/planilha/notificar', async (req, res) => {
  try {
    if (!SHEET_WEBHOOK_URL) {
      return res.json({ ok:false, erro:'SHEET_WEBHOOK_URL não configurado no Railway' });
    }
    const { nome, status } = req.body || {};
    if (!nome || !status) return res.status(400).json({ ok:false, erro:'nome ou status ausente' });
    const resultado = await httpsPostSeguindoRedirect(SHEET_WEBHOOK_URL, { nome, status });
    res.json(resultado);
  } catch(e) {
    console.error('Erro notificando planilha:', e.message);
...
app.get('/api/bling/situacoes', async (req, res) => {
  ...
  try {
    const data = await httpsGet('api.bling.com.br', '/Api/v3/situacoes/modulos/0',
...
app.get('/api/bling/reimportar/:num', async (req, res) => {
  ...
  try {
    ...
    if (!etiqPed.municipio && det.data.contato?.id) {
      try {
        const cont = await httpsGet('api.bling.com.br', `/Api/v3/contatos/${det.data.contato.id}`,
          { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' });
...
app.get('/api/bling/debugcontato/:num', async (req, res) => {
  ...
  try {
    ...
    const cont = await httpsGet('api.bling.com.br',
...
async function nomeVendedor(token, vendedorId) {
  ...
  try {
    const r = await httpsGet('api.bling.com.br', `/Api/v3/vendedores/${vendedorId}`,
      { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' });
...
// ID da situação "Cancelado" no Bling. Confirmado em 14/08/2026 (Wesley + debug
// de um pedido real): é o padrão de fábrica da conta, já estava até na lista de
// idsSituacoes que o sync usa pra buscar pedidos — só nunca era checado depois.
const BLING_SITUACAO_CANCELADO_ID = 12;

// Um pedido do Bling pode virar mais de 1 registro local (1 por colchão do
// pedido), todos com o mesmo pedidoNum. Pra cancelar certo, precisa achar todos.
function pedidosDoNumero(db, numPedido) {
  return db.pedidos.filter(x => x.pedidoNum === numPedido);
}

// ── Cancelamento propagado do Bling — FONTE ÚNICA ───────────────────────────
// Se o pedido já existe aqui e a situação dele no Bling é Cancelado, cancela
// aqui também. De propósito, só anda numa direção (Bling cancelado → sistema
// cancelado). Reativação continua manual pelo botão "↺ Reativar" — decisão
// tomada com o Wesley em 14/08/2026, pra evitar reativar pedido por engano
// numa falha de sincronização.
function sincronizarSituacaoCancelamento(db, numPedido, situacaoBling) {
  if (!situacaoBling) return 0;
  const canceladoNoBling = Number(situacaoBling.id) === BLING_SITUACAO_CANCELADO_ID;
  if (!canceladoNoBling) return 0;
  const locais = pedidosDoNumero(db, numPedido);
  let mudou = 0;
  for (const ped of locais) {
    if (ped.status !== 'cancelado') {
      ped.status = 'cancelado';
      ped.canceladoViaBling = true;
...
function parsearPedidoBling(p) {
  ...
  // Pedido que já chega cancelado no Bling (ex: importação atrasada de um
  // pedido criado e cancelado no mesmo dia) não pode nascer como "pendente"
  // aqui. Ver BLING_SITUACAO_CANCELADO_ID.
  const statusInicial = (p.situacao && Number(p.situacao.id) === BLING_SITUACAO_CANCELADO_ID)
    ? 'cancelado' : 'pendente';
...
app.get('/api/bling/sync', async (req, res) => {
  ...
  try {
    ...
    let todosPedidos = [];
    while(true) {
      const data = await httpsGet('api.bling.com.br',
        ...
        { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' });
      // A Bling pode responder com um objeto de erro em vez da lista de pedidos
      // (token inválido, permissão revogada, limite de chamadas etc). Antes,
      // isso caía direto pro "break" e virava "nenhum pedido encontrado" sem
      // avisar o motivo real. Agora, se vier um erro explícito da Bling, para
      // aqui e devolve o motivo de verdade em vez de mascarar como zero pedidos.
      if (data && data.error) {
        const motivoBling = data.error.message || data.error.type || JSON.stringify(data.error);
        return res.json({ ok:false, erro: `Bling recusou a chamada: ${motivoBling}` });
      }
    ...
    if (!todosPedidos.length) return res.json({ ok:false, erro: 'Nenhum pedido encontrado no Bling (a Bling respondeu normalmente, mas sem nenhum pedido nas situações/período filtrados)' });

    const db = await lerDB();
    let novos = 0, atualizados = 0, canceladosAuto = 0;
    ...
    for (const pedBling of todosPedidos) {
      ...
      if (jaExiste) {
        atualizados++;
        // Pedido já importado — checa se a situação no Bling virou Cancelado
        // e, se sim, cancela aqui também. Não busca detalhe completo pra isso,
        // o campo situacao já vem na listagem.
        canceladosAuto += sincronizarSituacaoCancelamento(db, numPedido, pedBling.situacao);
        continue;
      }
      if (pedidoIgnorado(db, numPedido)) continue; // excluído por decisão — não reimportar

      // Buscar detalhes do pedido
      const det = await httpsGet('api.bling.com.br', `/Api/v3/pedidos/vendas/${pedBling.id}`,
        { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' });
      ...
      if (!etiqPed.municipio && pedData.contato?.id) {
        try {
          const cont = await httpsGet('api.bling.com.br', `/Api/v3/contatos/${pedData.contato.id}`,
            { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' });
    ...
    if (novos > 0 || canceladosAuto > 0) await salvarDB(db);
    res.json({ ok:true, novos, atualizados, canceladosAuto, total: todosPedidos.length });
  } catch(e) {
    res.status(500).json({ ok:false, erro: e.message });
  }
});
...
// Conciliação bancária: "de-para" nome do extrato → categoria real. Nasceu da
// investigação do furo de julho/2026 (Rohi Móveis = ADS, Luiz = aluguel,
// Wesley Kleber = Chaperini, etc.) — salvo aqui pra não precisar reexplicar
// tudo de novo todo mês.
async function garantirSeedConciliacao(db) {
  if (!db.conciliacaoBanco) db.conciliacaoBanco = [];
  // Mescla — nunca sobrescreve o que já existe, só adiciona nome novo do
  // catálogo de referência que ainda não está lá. Assim, toda vez que a gente
  // descobre um nome novo (tipo os freteiros hoje), ele chega sozinho no
  // próximo deploy, sem apagar nada que você já tenha customizado na tela.
  const nomesExistentes = new Set(db.conciliacaoBanco.map(c => (c.nomeBanco||'').trim().toLowerCase()));
  let adicionados = 0;
  for (const item of SEED_CONCILIACAO_BANCO) {
    const chave = (item.nomeBanco||'').trim().toLowerCase();
    if (!nomesExistentes.has(chave)) {
      db.conciliacaoBanco.push(JSON.parse(JSON.stringify(item)));
  ...
  if (adicionados > 0) {
    await salvarDB(db);
    console.log(`✅ Conciliação bancária: ${adicionados} nome(s) novo(s) do catálogo de referência mesclado(s).`);
  }
  return db;
}
...
app.get('/api/dados', async (req, res) => {
  try {
    ...
    db = await garantirSeedConciliacao(db);
...
app.post('/api/salvar', async (req, res) => {
  try {
    ...
    // Logo do sistema — string única (base64), sobrescreve direto, sem checagem
    // de "redução drástica" (não faz sentido pra esse campo, é 1 valor só).
    if (body.logoBase64 !== undefined) {
      db.logoBase64 = body.logoBase64;
    }
    // Conciliação bancária — mesma proteção contra redução drástica do catalogoSku.
    if (body.conciliacaoBanco !== undefined) {
      const atual = (db.conciliacaoBanco || []).length;
      const novo = (body.conciliacaoBanco || []).length;
      if (atual > 5 && novo < atual * 0.5) {
        console.error(`🚨 BLOQUEADO: tentativa de reduzir conciliacaoBanco de ${atual} para ${novo}`);
        return res.status(400).json({ ok: false, erro: 'Payload suspeito bloqueado (redução de conciliacaoBanco)' });
      }
      db.conciliacaoBanco = body.conciliacaoBanco;
    }
...
// ══════════════════════════════ PONTO ELETRÔNICO ════════════════════════════
// IMPORTANTE — "pontos" propositalmente NUNCA entra na whitelist do /api/salvar
// acima. Todo o resto do sistema aceita o cliente mandar o array pronto porque
// é dado operacional (pedido, custo, etc.) — ponto é registro legal de jornada,
// então o HORÁRIO tem que ser sempre o do SERVIDOR no momento da batida, nunca
// um valor que o navegador do colaborador possa mandar. Se "pontos" entrasse
// no /api/salvar genérico, qualquer um com o DevTools aberto poderia reescrever
// o histórico de ponto de qualquer dia — destruiria o único motivo de isso
// existir. Por isso os registros só entram por /api/ponto/bater (auto, hora do
// servidor) ou /api/ponto/corrigir (gestor, com motivo, sempre por adição —
// nunca edita/apaga o registro original).
app.get('/api/ponto/meu-ip', (req, res) => {
  const ip = ipCliente(req);
  res.json({ ip, autorizado: ipAutorizado(ip), fabricaConfigurada: FABRICA_IPS.length > 0 });
});

app.post('/api/ponto/bater', async (req, res) => {
  try {
    const { pin } = req.body || {};
    if (!pin) return res.status(400).json({ ok:false, erro:'pin_ausente', mensagem:'PIN não informado.' });
    const ip = ipCliente(req);
    if (!ipAutorizado(ip)) {
      return res.status(403).json({ ok:false, erro:'ip_nao_autorizado', ip, mensagem:'Esse ponto só pode ser batido conectado na rede Wi-Fi da fábrica.' });
    }
    const db = await lerDB();
    const colab = (db.colaboradores || []).find(c => c.pin === pin);
    if (!colab) return res.status(400).json({ ok:false, erro:'pin_invalido', mensagem:'PIN não encontrado.' });
    if (!db.pontos) db.pontos = [];

    const { data, horario } = agoraBrasil();
    const doDia = db.pontos.filter(p => p.colaborador === colab.nome && p.data === data);
    const ORDEM = ['entrada1','saida1','entrada2','saida2'];
    const jaTem = new Set(doDia.map(p => p.tipo));
    const proximo = ORDEM.find(t => !jaTem.has(t));
    if (!proximo) {
      return res.status(400).json({ ok:false, erro:'dia_completo', mensagem:'Você já bateu os 4 pontos de hoje.', doDia });
    }
    ...
    await salvarDB(db);
    res.json({ ok:true, registro, doDia: [...doDia, registro] });
  } catch(e) {
    console.error('Erro em /api/ponto/bater:', e);
    res.status(500).json({ ok:false, erro:'erro_interno', mensagem: e.message });
  }
});

// Pro colaborador ver, ao logar, quais dos 4 pontos de hoje já estão feitos.
app.get('/api/ponto/dia', async (req, res) => {
  try {
    const { pin, data } = req.query;
    if (!pin) return res.status(400).json({ ok:false, erro:'pin_ausente' });
    const db = await lerDB();
    const colab = (db.colaboradores || []).find(c => c.pin === pin);
    if (!colab) return res.status(400).json({ ok:false, erro:'pin_invalido' });
    const dataAlvo = data || agoraBrasil().data;
    const registros = (db.pontos || []).filter(p => p.colaborador === colab.nome && p.data === dataAlvo);
    res.json({ ok:true, colaborador: colab.nome, data: dataAlvo, registros });
  } catch(e) { res.status(500).json({ ok:false, erro:'erro_interno', mensagem: e.message }); }
});

// Relatório do gestor — todos os registros de um mês, de todo mundo. A tela
// que usa isso é responsabilidade do gestor filtrar/mostrar; a API só entrega
// o dado cru (mesmo padrão do resto do sistema: proteção real é organizacional
// no frontend, não tem checagem de "é gestor mesmo" aqui no servidor ainda —
// mesma limitação já registrada no roadmap pro resto do sistema de permissão).
app.get('/api/ponto/relatorio', async (req, res) => {
  try {
    const db = await lerDB();
    const mes = req.query.mes || agoraBrasil().data.slice(0,7);
    const registros = (db.pontos || []).filter(p => p.data && p.data.startsWith(mes));
    res.json({ ok:true, mes, registros });
  } catch(e) { res.status(500).json({ ok:false, erro:'erro_interno', mensagem: e.message }); }
});

// Correção manual do gestor (esqueceu de bater, celular sem sinal, etc.).
// Nunca edita ou apaga o registro original — só ADICIONA um novo marcado como
// corrigido, com motivo e quem corrigiu. Preserva o histórico íntegro: se um
// dia isso precisar servir de prova em alguma discussão trabalhista, mexer no
// dado original destruiria a credibilidade dele.
app.post('/api/ponto/corrigir', async (req, res) => {
  try {
    const { colaborador, data, tipo, horario, motivo, corrigidoPor } = req.body || {};
    if (!colaborador || !data || !tipo || !horario || !motivo) {
      return res.status(400).json({ ok:false, erro:'Preencha colaborador, data, tipo, horário e motivo da correção.' });
    }
    const db = await lerDB();
    if (!db.pontos) db.pontos = [];
    const registro = {
      id: 'pt_' + Date.now() + '_' + Math.random().toString(36).slice(2,7),
      colaborador, data, tipo, horario,
      ts: new Date(`${data}T${horario}`).getTime(),
      ip: ipCliente(req),
      corrigido: true,
      motivo,
      corrigidoPor: corrigidoPor || 'Gestor',
    ...
    console.error('Erro em /api/ponto/corrigir:', e);
    res.status(500).json({ ok:false, erro:'erro_interno', mensagem: e.message });
  }
});

// ── Reset de teste ───────────────────────────────────────────────────────────
// Só pra limpar os testes de configuração (antes do ponto valer de verdade).
// Depois que virar oficial, evite usar isso pra apagar registro real — some com
// o dado, não fica rastro nenhum de que existiu. É diferente da correção acima
// (que sempre soma um registro novo, nunca apaga o antigo).
app.post('/api/ponto/apagar-dia', async (req, res) => {
  try {
    const { colaborador, data } = req.body || {};
    if (!colaborador || !data) return res.status(400).json({ ok:false, erro:'Informe colaborador e data.' });
    const db = await lerDB();
    const antes = (db.pontos || []).length;
    db.pontos = (db.pontos || []).filter(p => !(p.colaborador === colaborador && p.data === data));
    await salvarDB(db);
    res.json({ ok:true, removidos: antes - db.pontos.length });
  } catch(e) { res.status(500).json({ ok:false, erro:'erro_interno', mensagem: e.message }); }
});

app.post('/api/ponto/apagar-tudo', async (req, res) => {
  try {
    if (req.body?.confirmar !== 'APAGAR TUDO') {
      return res.status(400).json({ ok:false, erro:'Confirmação ausente.' });
    }
    const db = await lerDB();
    const antes = (db.pontos || []).length;
    db.pontos = [];
...
// ── Autorização de saída antecipada sem desconto ─────────────────────────────
// NÃO muda o horário real batido — só adiciona uma marcação em cima do registro
// dizendo "gestor autorizou, não descontar". O horário que a pessoa bateu de
// verdade continua exatamente como foi batido. Isso existe de propósito: mudar
// o horário em si (fazer o sistema achar que ela saiu mais tarde do que saiu)
// é adulterar registro de jornada — isso sim é o tipo de coisa que gera processo
// trabalhista sério, e pior: se um registro for pego adulterado, a Justiça
// tende a desconfiar do sistema inteiro, não só daquele dia.
app.post('/api/ponto/autorizar', async (req, res) => {
  try {
    const { id, motivo, autorizadoPor } = req.body || {};
    if (!id || !motivo) return res.status(400).json({ ok:false, erro:'Informe o registro e o motivo da autorização.' });
    const db = await lerDB();
    const reg = (db.pontos || []).find(p => p.id === id);
    if (!reg) return res.status(404).json({ ok:false, erro:'Registro não encontrado.' });
    reg.autorizado = true;
    reg.autorizadoMotivo = motivo;
    reg.autorizadoPor = autorizadoPor || 'Gestor';
    reg.autorizadoEm = Date.now();
    await salvarDB(db);
...
// POST /api/extrato/analisar — recebe o TEXTO colado de um extrato bancário
// (várias transações de uma vez, não uma foto só) e devolve uma lista de
// propostas de lançamento avulso. Reaproveita a mesma tabela de conciliação
// bancária (nome do banco → categoria real) já usada na investigação de
// julho/2026, pra manter a mesma classificação consistente mês a mês — e só
// pede pra IA classificar o que ainda não está mapeado ali.
// Nada é salvo no DRE aqui — só devolve a proposta. Wesley aprova/rejeita
// linha por linha (ou em lote) antes de qualquer coisa virar avulso de verdade.
app.post('/api/extrato/analisar', async (req, res) => {
  try {
    const { texto, pdfBase64 } = req.body;
    if ((!texto || !texto.trim()) && !pdfBase64) {
      return res.status(400).json({ ok:false, erro:'anexa o PDF ou cola o texto do extrato' });
    }
    if (!ANTHROPIC_API_KEY) return res.status(500).json({ ok:false, erro:'ANTHROPIC_API_KEY não configurada no servidor' });

    const db = await lerDB();
    const deParaConhecido = (db.conciliacaoBanco || [])
      .map(c => `- "${c.nomeBanco}" → ${c.categoria}${c.obs ? ' ('+c.obs+')' : ''}`)
      .join('\n');

    const instrucoes = `Você está lendo um extrato bancário (PagBank ou Santander) de uma fábrica de colchões (Orto Premium), pra separar despesas pequenas do dia a dia por categoria, já que hoje isso é feito só no fim do mês, tudo misturado.

TABELA DE NOMES JÁ CONHECIDOS (use essa classificação sempre que o nome bater, não invente outra):
${deParaConhecido || '(nenhum nome mapeado ainda)'}

CATEGORIAS FIXAS DO DRE (use uma dessas quando fizer sentido — cada uma já tem campo próprio no sistema, não deveria virar "avulso"): ${CATEGORIAS_DRE.join(', ')}.
Se não se encaixar em nenhuma categoria fixa, categoriaSugerida = "avulso" com uma tagCurta descrevendo o gasto (ex: "Combustível", "Alimentação", "Manutenção", "Consumível produção").

REGRAS IMPORTANTES:
- Pegue SÓ transações de SAÍDA (gasto/pagamento) — ignore "Vendas - Disponível", "Saldo do dia", qualquer entrada de dinheiro.
- Ignore transferência pra "Orto Premium Colchao Ltda" (é movimentação entre contas da própria empresa, não é gasto).
- ⚠️ PAGAMENTO A TRANSPORTADORA/FRETEIRO NUNCA DEVE VIRAR AVULSO — o custo de frete já é calculado automaticamente por pedido no sistema de cotação de frete, separado disso aqui. Se o nome bater com "frete" na tabela de referência acima, OU parecer claramente uma transportadora/freteiro pelo nome (ex: "Transportes", nome de pessoa física recebendo valores variáveis e frequentes de forma consistente com entrega), marque categoriaSugerida="frete" e jaContabilizadoAutomaticamente=true. Isso NÃO deve ser aprovado como avulso — o objetivo é só te mostrar que existe, não fazer você lançar de novo o que já é automático.
- Marque provavelmentePessoal=true pra coisas que parecem consumo pessoal do dono (restaurante, ifood, lanchonete, padaria/panificadora avulsa, farmácia, entretenimento) — não pra material de fábrica, frete, ou fornecedor.
- Nunca invente valor ou data — se a linha estiver ambígua, confianca="baixa" e explica em observacao.

Responda SOMENTE com um JSON array válido, sem markdown, sem texto antes/depois, cada item no formato exato:
{"data":"AAAA-MM-DD","descricao":"string curta","valor":0.00,"categoriaSugerida":"uma das fixas ou 'avulso'","tagCurta":"só se for avulso — nome curto da categoria, ex Combustível","provavelmentePessoal":false,"jaContabilizadoAutomaticamente":false,"confianca":"alta|media|baixa","observacao":""}`;

    // PDF anexado: manda o arquivo de verdade pra API (ela lê a tabela como
    // está no documento, sem depender do navegador extrair texto — é
    // exatamente esse passo de "copiar e colar" que embaralhava a ordem das
    // colunas em PDF de tabela e quebrava a leitura antes.
    let content;
    if (pdfBase64) {
      content = [
    ...
    const respIA = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: 'claude-sonnet-4-6',
        // Um mês cheio de extrato (visto no Santander: até ~300 lançamentos
        // só de cartão, fora PIX) gera uma lista longa de JSON. 64000 é o
        // teto real que o Sonnet aguenta na API síncrona (128k é só o Opus).
        // Mesmo assim, meses muito grandes (ex: janeiro) podem passar disso —
        // nesse caso divide em quinzena, o /api/tributaria/salvar já soma
        // em vez de substituir (16/08/2026).
        max_tokens: 64000,
    ...
    const dataIA = await respIA.json();
    if (dataIA.error) {
      console.error('❌ Erro Anthropic API (extrato):', JSON.stringify(dataIA.error).slice(0,300));
      return res.status(500).json({ ok:false, erro: 'Falha ao analisar extrato: ' + (dataIA.error.message || 'erro desconhecido') });
    }
    if (dataIA.stop_reason === 'max_tokens') {
      // A resposta foi cortada no meio mesmo com o limite maior — melhor
      // avisar claramente do que devolver uma lista incompleta calada.
      console.warn('⚠️ Resposta da IA cortada por max_tokens no /api/extrato/analisar — mês com volume grande demais pra 1 chamada só.');
      return res.status(500).json({ ok:false, erro: 'Esse período tem transações demais pra analisar de uma vez. Tenta dividir em pedaços menores (ex: quinzena em vez de mês inteiro).' });
    }
    const textoResposta = (dataIA.content || []).map(b => b.text || '').join('').trim();
    let transacoes;
    try {
      ...
      // A IA às vezes escreve uma frase antes/depois do array mesmo pedindo
      // pra não fazer isso — tenta pegar só o trecho entre o primeiro [ e o
      // último ] antes de desistir de vez.
      try {
        const inicio = textoResposta.indexOf('[');
        const fim = textoResposta.lastIndexOf(']');
        if (inicio === -1 || fim === -1 || fim < inicio) throw e1;
        transacoes = JSON.parse(textoResposta.slice(inicio, fim + 1));
      } catch(e2) {
        console.error('❌ IA não devolveu JSON válido (extrato). Resposta completa:', textoResposta);
        return res.status(500).json({ ok:false, erro:'A IA não conseguiu estruturar o extrato. Tenta colar de novo, ou em pedaços menores.' });
      }
    }
    if (!Array.isArray(transacoes)) {
      console.error('❌ Resposta da IA não é um array (extrato):', textoResposta.slice(0,500));
      return res.status(500).json({ ok:false, erro:'A IA devolveu um formato inesperado. Tenta colar de novo, ou em pedaços menores.' });
    }

    // Checagem de segurança adicional (não confia só na IA pra essa regra
    // específica, porque errar aqui duplica custo de frete de verdade no DRE):
    // qualquer nome que já está marcado como "frete" na Conciliação Bancária
    // é forçado como já-contabilizado, mesmo que a IA não tenha marcado certo.
    const nomesFrete = (db.conciliacaoBanco || [])
      .filter(c => c.categoria === 'frete')
      .map(c => (c.nomeBanco || '').trim().toLowerCase());
    transacoes.forEach(t => {
      const descNorm = (t.descricao || '').toLowerCase();
      if (nomesFrete.some(nome => nome && descNorm.includes(nome))) {
        t.categoriaSugerida = 'frete';
        t.jaContabilizadoAutomaticamente = true;
      }
    });

    res.json({ ok:true, transacoes, total: transacoes.length });
  } catch(e) {
    console.error('❌ Erro em /api/extrato/analisar:', e.message);
    res.status(500).json({ ok:false, erro: e.message });
  }
});
...
// ═══════════════════════════════════════════════════════════════════════════
// ANÁLISE TRIBUTÁRIA (Fase 2 — 14-16/08/2026) — projeto à parte da consultoria
// de Lucro Real. Propositalmente SEPARADO do fluxo de "Extrato semanal" acima:
// aquele fluxo existe pra pegar só o que ainda NÃO está contabilizado (por
// isso ele esconde frete e matéria-prima, que já são calculados em outro
// lugar do sistema). Aqui é o oposto: precisamos ver e guardar TUDO — matéria-
// prima, frete, folha, empréstimo, pessoal, tudo — porque é isso que vai pra
// consultoria decidir sobre migração pra Lucro Real. Reaproveitar o endpoint
// antigo pra isso teria feito a análise tributária "sumir" com matéria-prima
// e frete sem ninguém perceber.
// ═══════════════════════════════════════════════════════════════════════════

// Jobs de análise tributária em andamento, em memória (não precisa persistir
// em disco — é só enquanto a análise roda, alguns minutos no máximo).
// Guarda por até 1h depois de concluído, pra não vazar memória com o tempo.
const _tribJobs = new Map();
function _tribLimparJobsAntigos() {
  const umaHora = 60 * 60 * 1000;
  const agora = Date.now();
  for (const [id, job] of _tribJobs) {
    if (job.criadoEm && (agora - job.criadoEm) > umaHora) _tribJobs.delete(id);
  }
}

// POST /api/tributaria/analisar — recebe o PDF de 1 mês inteiro (extrato
// completo, Santander ou PagBank) e devolve TODAS as transações — entrada E
// saída — já com a categoria/natureza fiscal conhecida (via Conciliação
// Bancária) quando bater o nome, ou proposta da IA quando não bater.
// Não salva nada ainda — só devolve a proposta pra Wesley revisar.
//
// RODA EM SEGUNDO PLANO (16/08/2026): meses grandes (ex: janeiro, ~300+
// transações) demoram mais que o tempo que o proxy do Railway espera por uma
// resposta só, e a conexão cai com "upstream error" mesmo com o processamento
// certo por trás. Solução: essa rota devolve um jobId NA HORA, processa a
// análise de verdade em segundo plano, e o navegador fica perguntando o
// status em /api/tributaria/analisar/status/:jobId de tempos em tempos. Assim
// nenhuma conexão HTTP fica esperando minutos — cada chamada é rápida.
app.post('/api/tributaria/analisar', async (req, res) => {
  const { pdfBase64, texto, mes, banco } = req.body;
  if ((!texto || !texto.trim()) && !pdfBase64) {
    return res.status(400).json({ ok:false, erro:'anexa o PDF ou cola o texto do extrato' });
  }
  if (!mes) return res.status(400).json({ ok:false, erro:'informa o mês (AAAA-MM) que esse extrato cobre' });
  if (!ANTHROPIC_API_KEY) return res.status(500).json({ ok:false, erro:'ANTHROPIC_API_KEY não configurada no servidor' });

  _tribLimparJobsAntigos();
  const jobId = 'trib_' + Date.now() + '_' + Math.random().toString(36).slice(2,8);
  _tribJobs.set(jobId, { status: 'processando', criadoEm: Date.now(), mes, banco });

  // Responde JÁ, não espera a IA terminar
  res.json({ ok:true, jobId });

  // Processa em segundo plano — o navegador não está mais esperando essa parte
  (async () => {
    try {
      const db = await lerDB();
      const deParaConhecido = (db.conciliacaoBanco || [])
        .map(c => `- "${c.nomeBanco}" → categoria: ${c.categoria}${c.naturezaFiscal ? ', natureza fiscal: '+c.naturezaFiscal+(c.obsFiscal?' ('+c.obsFiscal+')':'') : ''}${c.obs ? ' — ' + c.obs : ''}`)
        .join('\n');

      const instrucoes = `Você está lendo o extrato bancário de 1 mês inteiro (PagBank ou Santander) de uma fábrica de colchões (Orto Premium), pra uma análise tributária completa que vai embasar a decisão de migrar ou não pra Lucro Real em 2027. É pra uma consultoria externa analisar, então precisão importa mais que velocidade.

TABELA DE NOMES JÁ CONHECIDOS (use essa classificação sempre que o nome bater — a "natureza fiscal", quando existir, é a que importa pra esse relatório, não a categoria operacional):
${deParaConhecido || '(nenhum nome mapeado ainda)'}

DIFERENÇA IMPORTANTE EM RELAÇÃO A OUTRAS ANÁLISES DESSE SISTEMA: aqui você NÃO deve pular nem esconder nada. Inclua TODA transação, de entrada e de saída — matéria-prima, frete, folha, empréstimo, tudo. O objetivo aqui é o oposto de evitar duplicidade: é ver o quadro completo.

⚠️ ATENÇÃO ESPECIAL EM ENTRADA (erro já aconteceu num teste anterior — a resposta saiu só com saída, isso está ERRADO): um extrato bancário de verdade sempre tem crédito e débito misturados linha por linha. "PIX RECEBIDO", "TED RECEBIDO", "CRÉDITO", "DEPÓSITO" são ENTRADA. Percorra o documento inteiro, do início ao fim, e garanta que toda linha de crédito também vire um item no array de resposta, não só as de débito.

⚠️ ATENÇÃO ESPECIAL — NÃO DUPLICAR ENTRE AS DUAS CONTAS DA MESMA EMPRESA: a Orto Premium tem conta no Santander E no PagBank. A maquininha de cartão recebe a venda no PagBank; boa parte do que sobra lá é transferida pro Santander pra pagar folha/matéria-prima/reserva. Se uma linha for PIX/TED de "ORTO PREMIUM COLCHAO LTDA" pra ela mesma (entrando no Santander vindo do PagBank, ou vice-versa — mesmo CNPJ nos dois lados), isso é categoria=movimentacao_interna, NUNCA categoria de receita/venda. Contar isso como entrada real duplicaria a mesma venda duas vezes (uma quando entrou no PagBank pela maquininha, outra quando foi transferida pro Santander). Isso vale tanto analisando o extrato do Santander quanto do PagBank.

CATEGORIAS A USAR (categoria):
materia_prima, frete, folha, ads_marketing, aluguel_fixo, emprestimo, investimento_aplicacao, distribuicao_lucros, transferencia_pessoal_socio, imposto, tarifa_bancaria, venda_cliente (entrada real de venda, ex: recebimento da maquininha de cartão), movimentacao_interna (transferência entre contas da própria empresa, ou aplicação automática do próprio banco tipo ContaMax — dinheiro do próprio caixa indo e voltando, não é gasto nem receita real), avulso (quando não se encaixar em nenhuma).

REGRAS:
- Classifique TANTO entrada quanto saída, sem exceção. Campo "tipo": "entrada" ou "saida".
- Se o nome bater com a tabela de referência, use a categoria/natureza fiscal de lá, mesmo que pareça "pequeno demais pra importar" — é justamente pra não perder nada que essa análise existe.
- Marque provavelmentePessoal=true pra transferência que parece consumo pessoal do dono ou sócio (não confundir com pró-labore ou distribuição de lucro formalizados — esses têm categoria própria).
- "APLICACAO CONTAMAX", "RESGATE CONTAMAX AUTOMATICO" e equivalentes (aplicação automática do próprio banco) são categoria=movimentacao_interna.
- Nunca invente valor, data ou nome — se a linha estiver ambígua ou sem nome identificável, confianca="baixa" e explica em observacao. Não force uma categoria só pra não deixar em branco.
- Nunca invente CPF/CNPJ.
- ANTES DE RESPONDER: confira se sua lista final tem pelo menos algumas transações com tipo="entrada". Se sua resposta só tiver "saida", você pulou linhas do documento — volte e inclua as que faltaram.

Responda SOMENTE com um JSON array válido, sem markdown, sem texto antes/depois, cada item no formato exato:
{"data":"AAAA-MM-DD","tipo":"entrada|saida","descricao":"string curta com o nome completo se houver","valor":0.00,"categoria":"uma das da lista acima","naturezaFiscal":"só se a tabela de referência tiver essa informação pro nome, senão string vazia","provavelmentePessoal":false,"confianca":"alta|media|baixa","observacao":""}`;
      ...
      // Retry automático em falha de REDE (não de conteúdo) — "fetch failed"
      // pode ser instabilidade passageira de conexão Railway↔Anthropic, não
      // vale obrigar o Wesley a reprocessar o mês inteiro por causa disso.
      let respIA, ultimoErroRede;
      for (let tentativa = 1; tentativa <= 2; tentativa++) {
        try {
          ...
          console.warn(`⚠️ Falha de rede na tentativa ${tentativa}/2 (tributária):`, erroRede.message, erroRede.cause ? '| causa: '+(erroRede.cause.message||erroRede.cause.code||erroRede.cause) : '');
          if (tentativa < 2) await new Promise(r => setTimeout(r, 3000));
        }
      }
      if (ultimoErroRede) {
        const causa = ultimoErroRede.cause ? (ultimoErroRede.cause.message || ultimoErroRede.cause.code || JSON.stringify(ultimoErroRede.cause)) : '';
        _tribJobs.set(jobId, { status:'erro', criadoEm: Date.now(), erro: `Falha de conexão com a IA depois de 2 tentativas${causa ? ' ('+causa+')' : ''}. Tenta de novo em alguns minutos.` });
        return;
      }

      const dataIA = await respIA.json();
      if (dataIA.error) {
        console.error('❌ Erro Anthropic API (tributária):', JSON.stringify(dataIA.error).slice(0,300));
        _tribJobs.set(jobId, { status:'erro', criadoEm: Date.now(), erro: 'Falha ao analisar extrato: ' + (dataIA.error.message || 'erro desconhecido') });
        return;
      }
      if (dataIA.stop_reason === 'max_tokens') {
        console.warn('⚠️ Resposta cortada por max_tokens no /api/tributaria/analisar.');
        _tribJobs.set(jobId, { status:'erro', criadoEm: Date.now(), erro: 'Esse mês tem transações demais pra analisar de uma vez. Tenta dividir em duas quinzenas.' });
        return;
      }
      ...
      try {
        ...
        try {
          ...
          console.error('❌ IA não devolveu JSON válido (tributária). Resposta completa:', textoResposta);
          _tribJobs.set(jobId, { status:'erro', criadoEm: Date.now(), erro:'A IA não conseguiu estruturar o extrato. Tenta de novo.' });
          return;
        }
      }
      if (!Array.isArray(transacoes)) {
        _tribJobs.set(jobId, { status:'erro', criadoEm: Date.now(), erro:'A IA devolveu um formato inesperado. Tenta de novo.' });
        return;
      }

      transacoes.forEach((t, i) => { t._tempId = mes + '_' + i; t.banco = banco || ''; });
      _tribJobs.set(jobId, { status:'concluido', criadoEm: Date.now(), mes, transacoes });
    } catch(e) {
      console.error('❌ Erro em segundo plano /api/tributaria/analisar:', e.message, e.cause ? '| causa: '+(e.cause.message||e.cause.code||e.cause) : '');
      const causa = e.cause ? (e.cause.message || e.cause.code || '') : '';
      _tribJobs.set(jobId, { status:'erro', criadoEm: Date.now(), erro: e.message + (causa ? ' ('+causa+')' : '') });
    }
  })();
});

// GET /api/tributaria/analisar/status/:jobId — o navegador consulta esse
// endpoint de tempos em tempos até o job dar "concluido" ou "erro".
app.get('/api/tributaria/analisar/status/:jobId', (req, res) => {
  const job = _tribJobs.get(req.params.jobId);
  if (!job) return res.status(404).json({ ok:false, erro:'Job não encontrado (pode ter expirado — mais de 1h).' });
  res.json({ ok:true, ...job });
});

// POST /api/tributaria/salvar — grava o mês inteiro de uma vez (Wesley já
// revisou/corrigiu na tela). Salva em bloco — 1 chamada, não 1 por transação,
// porque um mês tem 200-300+ linhas e o disco de dados não precisa disso.
// Chave é mes+banco, não só mes — Santander e PagBank do mesmo mês são fontes
// diferentes e não podem se substituir (bug encontrado e corrigido em
// 16/08/2026 antes do primeiro teste real, ver ROADMAP).
// Por padrão SOMA no que já existia (permite subir o mês em 2 partes, ex:
// quinzena 1 + quinzena 2, quando o mês inteiro de uma vez estoura o tempo
// de resposta e a plataforma corta com "upstream error" — descoberto e
// corrigido em 16/08/2026, ver ROADMAP). Faz um dedup simples por
// data+descricao+valor pra não duplicar se a mesma parte for salva 2x sem
// querer. Se precisar corrigir do zero, manda substituir:true.
app.post('/api/tributaria/salvar', async (req, res) => {
  try {
    const { mes, banco, transacoes, substituir } = req.body;
    if (!mes || !banco || !Array.isArray(transacoes)) {
      return res.status(400).json({ ok:false, erro:'mes, banco e transacoes (array) são obrigatórios' });
    }
    const db = await lerDB();
    if (!db.analiseTributaria) db.analiseTributaria = {};
    if (!db.analiseTributaria[mes]) db.analiseTributaria[mes] = {};

    const novas = transacoes.map(t => ({
      data: t.data, descricao: t.descricao, valor: Number(t.valor)||0, tipo: t.tipo === 'entrada' ? 'entrada' : 'saida',
      categoria: t.categoria || 'avulso', naturezaFiscal: t.naturezaFiscal || '',
      provavelmentePessoal: !!t.provavelmentePessoal, confianca: t.confianca || 'media', observacao: t.observacao || ''
    }));

    let listaFinal;
    if (substituir || !db.analiseTributaria[mes][banco]) {
      listaFinal = novas;
    } else {
      const existentes = db.analiseTributaria[mes][banco].transacoes || [];
      const chave = t => `${t.data}|${t.descricao}|${t.valor}`;
      const chavesExistentes = new Set(existentes.map(chave));
      const semDuplicata = novas.filter(t => !chavesExistentes.has(chave(t)));
      listaFinal = existentes.concat(semDuplicata);
    }

    db.analiseTributaria[mes][banco] = { transacoes: listaFinal, salvoEm: new Date().toISOString() };
    const ok = await salvarDB(db);
    res.json({ ok, totalSalvo: novas.length, totalAcumulado: listaFinal.length });
  } catch(e) {
    console.error('❌ Erro em /api/tributaria/salvar:', e.message);
    res.status(500).json({ ok:false, erro: e.message });
  }
});

// GET /api/tributaria/:mes — recupera um mês já salvo, com os dois bancos
// separados (pra conferir depois, ou pra Fase 4 exportar em PDF/Excel).
app.get('/api/tributaria/:mes', async (req, res) => {
  try {
    const db = await lerDB();
    const dadosMes = (db.analiseTributaria || {})[req.params.mes];
    if (!dadosMes) return res.status(404).json({ ok:false, erro:'Esse mês ainda não foi salvo.' });
    res.json({ ok:true, mes: req.params.mes, bancos: dadosMes });
  } catch(e) {
    res.status(500).json({ ok:false, erro: e.message });
  }
});

// GET /api/tributaria — lista todos os meses já salvos, com totais por banco
// (pra Fase 4 mostrar "quais meses já estão prontos" sem carregar tudo).
app.get('/api/tributaria', async (req, res) => {
  try {
    const db = await lerDB();
    const meses = Object.entries(db.analiseTributaria || {}).map(([mes, porBanco]) => ({
      mes,
      bancos: Object.entries(porBanco).map(([banco, dados]) => ({
        banco, total: (dados.transacoes||[]).length, salvoEm: dados.salvoEm
      }))
...
// DELETE /api/tributaria/:mes/:banco — apaga um mês/banco específico do zero
// (pra quando precisar reprocessar um mês inteiro de novo, em vez de somar
// em cima do que já tinha).
app.delete('/api/tributaria/:mes/:banco', async (req, res) => {
  try {
    const db = await lerDB();
    if (db.analiseTributaria && db.analiseTributaria[req.params.mes]) {
      delete db.analiseTributaria[req.params.mes][req.params.banco];
      if (Object.keys(db.analiseTributaria[req.params.mes]).length === 0) {
        delete db.analiseTributaria[req.params.mes];
      }
    }
    const ok = await salvarDB(db);
    res.json({ ok });
...
// Substituiu o WPPConnect (tentativa antiga, abandonada, nunca funcionou —
// ver histórico). Mesma tela/mesmo banco de dados de antes (whats-lista,
// db.whatsapp_mensagens, db.leads), só troca o "cano" de baixo.
async function evoRequest(metodo, caminho, body = null) {
  const url = EVOLUTION_API_URL + caminho;
...
function webhookUrlComSegredo() {
  const base = (process.env.MEU_DOMINIO || 'https://orto-producao-production.up.railway.app') + '/webhook/whatsapp';
  return EVOLUTION_WEBHOOK_SECRET ? `${base}?secret=${encodeURIComponent(EVOLUTION_WEBHOOK_SECRET)}` : base;
}

// DIAGNÓSTICO: testa a conexão básica com a Evolution API antes de tudo —
// mesma ideia do /api/bling/situacoes, pra nunca mais perder horas
// adivinhando se o problema é conexão/chave antes de checar o básico.
app.get('/api/evolution/diagnostico', async (req, res) => {
  if (!EVOLUTION_API_URL || !EVOLUTION_API_KEY) {
    return res.json({ ok:false, erro:'EVOLUTION_API_URL ou EVOLUTION_API_KEY não configurados no Railway' });
  }
  const r = await evoRequest('GET', '/instance/fetchInstances');
  res.json(r);
});

// Listar instâncias/números conectados
app.get('/api/evolution/instancias', async (req, res) => {
  const r = await evoRequest('GET', '/instance/fetchInstances');
  let instancias = [];
  if (r.ok && Array.isArray(r.data)) {
    instancias = r.data.map(inst => ({
      name: inst.name || inst.instanceName,
      instanceName: inst.name || inst.instanceName,
      connectionStatus: (inst.connectionStatus === 'open') ? 'open' : 'close',
      ownerJid: inst.ownerJid || inst.number || ''
    }));
  }
  res.json({ ok: r.ok, erro: r.ok ? undefined : (r.data?.response?.message || r.erro || 'falha ao listar'), data: instancias });
});
...
app.post('/api/evolution/criar-instancia', async (req, res) => {
  ...
  let r = await evoRequest('POST', '/instance/create', {
    instanceName: nome,
    qrcode: true,
    integration: 'WHATSAPP-BAILEYS',
    webhook: { url: webhookUrlComSegredo(), byEvents: false, events: ['MESSAGES_UPSERT'] }
  });

  // Instância já existe — não é erro de verdade, só busca o QR dela normalmente
  const jaExiste = !r.ok && (r.status === 403 || r.status === 409 ||
    JSON.stringify(r.data || '').toLowerCase().includes('already'));
  if (jaExiste) {
    r = await evoRequest('GET', `/instance/connect/${nome}`);
  }

  const qr = r.data?.qrcode?.base64 || r.data?.base64;
  res.json({ ok: r.ok || !!qr, data: r.data, qrcode: qr });
});

// Pegar/renovar QR Code
app.get('/api/evolution/qrcode/:nome', async (req, res) => {
  const nome = req.params.nome;
  const r = await evoRequest('GET', `/instance/connect/${nome}`);
  const qr = r.data?.qrcode?.base64 || r.data?.base64;
  res.json({ ok: r.ok, data: r.data, qrcode: qr });
});

// Status da conexão
app.get('/api/evolution/status/:nome', async (req, res) => {
  const nome = req.params.nome;
  const r = await evoRequest('GET', `/instance/connectionState/${nome}`);
  const estado = r.data?.instance?.state || r.data?.state || '';
  res.json({ ok: r.ok, data: { state: estado === 'open' ? 'open' : 'close', status: estado } });
});

// Remover/desconectar número
app.delete('/api/evolution/instancia/:nome', async (req, res) => {
  const nome = req.params.nome;
  const r = await evoRequest('DELETE', `/instance/logout/${nome}`);
  res.json({ ok: r.ok, data: r.data });
});

// Enviar mensagem de texto pelo próprio sistema (responde direto, sem
// precisar abrir o WhatsApp). Quem manda é o número da instância (o próprio
// vendedor), o sistema só está encaminhando o texto pra Evolution API.
app.post('/api/evolution/enviar', async (req, res) => {
  const { instancia, telefone, texto } = req.body || {};
  if (!instancia || !telefone || !texto?.trim()) {
    return res.status(400).json({ ok:false, erro:'instancia, telefone e texto são obrigatórios' });
  }
  const r = await evoRequest('POST', `/message/sendText/${instancia}`, {
    number: telefone,
    text: texto.trim()
  });
  if (!r.ok) return res.json({ ok:false, erro: r.data?.response?.message || r.data?.message || r.erro || 'falha ao enviar' });

  // Já grava a mensagem enviada aqui também (não espera o webhook confirmar),
  // pra aparecer na hora na conversa sem esperar o eco da Evolution.
  const db = await lerDB();
  if (!db.whatsapp_mensagens) db.whatsapp_mensagens = [];
  const agora = Math.floor(Date.now()/1000);
  db.whatsapp_mensagens.push({
    id: r.data?.key?.id || ('local_'+Date.now().toString(36)),
    instancia, telefone, fromMe: true, texto: texto.trim(), tipo: 'texto',
    timestamp: agora, nomeContato: ''
  });
  // Já marca como resolvida, já que você acabou de responder agora mesmo.
  if (!db.whatsapp_resolvidos) db.whatsapp_resolvidos = {};
  db.whatsapp_resolvidos[instancia+'|'+telefone] = { ateTimestamp: agora, resolvidoEm: Date.now() };
  await salvarDB(db);

  res.json({ ok:true, data: r.data });
});

// Transcreve UM áudio específico, sob demanda (só quando clicado, nunca
// automático — cada chamada tem custo real na OpenAI). Busca o áudio
// descriptografado na própria Evolution API (ela já sabe lidar com a parte
// de criptografia do WhatsApp, não precisamos reinventar isso) e manda pro
// Whisper da OpenAI virar texto.
app.post('/api/whatsapp/transcrever', async (req, res) => {
  const { instancia, telefone, messageId } = req.body || {};
  if (!instancia || !telefone || !messageId) {
    return res.status(400).json({ ok:false, erro:'instancia, telefone e messageId são obrigatórios' });
  }
  if (!OPENAI_API_KEY) {
    return res.json({ ok:false, erro:'OPENAI_API_KEY não configurado no Railway ainda' });
  }

  // 1. Busca o áudio já descriptografado na Evolution API
  const media = await evoRequest('POST', `/chat/getBase64FromMediaMessage/${instancia}`, {
    message: { key: { id: messageId } },
    convertToMp4: false
  });
  if (!media.ok || !media.data?.base64) {
    // Bug conhecido da própria Evolution API: áudio mandado de iPhone às
    // vezes falha aqui com erro de "ephemeralMessage" — não é coisa nossa,
    // é limitação relatada no projeto original, sem solução do lado de cá.
    const motivo = media.data?.response?.message?.[0] || media.data?.message || media.erro || 'não foi possível buscar o áudio';
    return res.json({ ok:false, erro: 'Evolution API: ' + motivo });
  }

  // 2. Manda o áudio pro Whisper (OpenAI) transcrever
  try {
    const mimetype = media.data.mimetype || 'audio/ogg';
    const ext = mimetype.includes('mp4') ? 'mp4' : mimetype.includes('mpeg') ? 'mp3' : 'ogg';
    const bufferAudio = Buffer.from(media.data.base64, 'base64');
    ...
    const dataWhisper = await respWhisper.json();
    if (!respWhisper.ok) {
      return res.json({ ok:false, erro: 'OpenAI: ' + (dataWhisper.error?.message || 'falha ao transcrever') });
    }
    const transcricao = (dataWhisper.text || '').trim();

    // 3. Salva o texto transcrito na própria mensagem, pra nunca mais
    // precisar pagar de novo pra ouvir esse mesmo áudio.
    const db = await lerDB();
    const msg = (db.whatsapp_mensagens || []).find(m => m.id === messageId && m.instancia === instancia && m.telefone === telefone);
    if (msg) {
      msg.transcricao = transcricao || '(áudio sem fala identificável)';
      await salvarDB(db);
    }
    res.json({ ok:true, transcricao: transcricao || '(áudio sem fala identificável)' });
  } catch(e) {
    res.json({ ok:false, erro: 'Erro ao transcrever: ' + e.message });
  }
});

// Descreve UMA imagem específica, sob demanda (só quando clicado). Usa a
// Claude (Anthropic), que já lê imagem nativamente — não precisa de conta
// nova nem chave nova, é a mesma ANTHROPIC_API_KEY que já roda no servidor.
app.post('/api/whatsapp/descrever-imagem', async (req, res) => {
  const { instancia, telefone, messageId } = req.body || {};
  if (!instancia || !telefone || !messageId) {
    return res.status(400).json({ ok:false, erro:'instancia, telefone e messageId são obrigatórios' });
  }
  if (!ANTHROPIC_API_KEY) {
    return res.json({ ok:false, erro:'ANTHROPIC_API_KEY não configurada no servidor' });
  }

  // 1. Busca a imagem na Evolution API (mesmo endpoint usado pro áudio, ele
  // serve qualquer tipo de mídia, não só áudio).
  const media = await evoRequest('POST', `/chat/getBase64FromMediaMessage/${instancia}`, {
    message: { key: { id: messageId } },
    convertToMp4: false
  });
  if (!media.ok || !media.data?.base64) {
    const motivo = media.data?.response?.message?.[0] || media.data?.message || media.erro || 'não foi possível buscar a imagem';
    return res.json({ ok:false, erro: 'Evolution API: ' + motivo });
  }

  // 2. Manda pra Claude descrever (modelo mais barato, é uma tarefa simples)
  try {
    const respIA = await fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: [
            { type: 'image', source: { type: 'base64', media_type: media.data.mimetype || 'image/jpeg', data: media.data.base64 } },
            { type: 'text', text: 'Descreva objetivamente, em português, em até 2 frases curtas, o que aparece nessa imagem trocada numa conversa de vendas de colchões. Se for print de conversa/documento, resuma o essencial do que está escrito.' }
          ]
        }]
      })
    });
    const dataIA = await respIA.json();
    if (dataIA.error) {
      return res.json({ ok:false, erro: 'Claude: ' + (dataIA.error.message || 'falha ao descrever') });
    }
    const descricao = (dataIA.content || []).map(b => b.text || '').join('').trim() || '(não foi possível descrever)';

    // 3. Salva na própria mensagem, pra nunca mais precisar pagar de novo
    const db = await lerDB();
    const msg = (db.whatsapp_mensagens || []).find(m => m.id === messageId && m.instancia === instancia && m.telefone === telefone);
    if (msg) {
      msg.descricaoImagem = descricao;
      await salvarDB(db);
    }
    res.json({ ok:true, descricao });
  } catch(e) {
    res.json({ ok:false, erro: 'Erro ao descrever: ' + e.message });
  }
});

// ── Leitura das conversas guardadas (auditoria) ──
// Lista uma linha por contato (instância + telefone), com a última mensagem,
// pra montar a lista tipo "caixa de entrada" na tela.
app.get('/api/whatsapp/conversas', async (req, res) => {
  const db = await lerDB();
  const msgs = db.whatsapp_mensagens || [];
  const resolvidos = db.whatsapp_resolvidos || {};
  const porContato = {};
  for (const m of msgs) {
    const chave = (m.instancia||'?') + '|' + (m.telefone||'?');
    if (!porContato[chave]) porContato[chave] = { ultima: null, ultimaDoCliente: null };
    const g = porContato[chave];
    if (!g.ultima || m.timestamp > g.ultima.timestamp) g.ultima = m;
    // O nome do contato só pode vir de mensagem que o CLIENTE mandou — se vier
    // de mensagem sua, o campo traz o nome do seu próprio perfil de WhatsApp
    // ("Orto Premium"), não o do cliente. Foi isso que bagunçou os nomes.
    if (!m.fromMe && (!g.ultimaDoCliente || m.timestamp > g.ultimaDoCliente.timestamp)) g.ultimaDoCliente = m;
  }
  const lista = Object.entries(porContato).map(([chave, g]) => {
    const m = g.ultima;
    const marcadoResolvido = resolvidos[chave];
    // Aguardando resposta = a mensagem mais recente foi do cliente E (nunca foi
    // marcada como resolvida OU o cliente escreveu de novo depois de resolvida).
    // Assim, "marcar como resolvida" limpa a pendência atual, mas se o cliente
    // voltar a escrever depois, some sozinho da lista de resolvidas.
    const aguardaResposta = !!(m && !m.fromMe && (!marcadoResolvido || m.timestamp > marcadoResolvido.ateTimestamp));
    return {
      instancia: m.instancia, telefone: m.telefone,
      nomeContato: g.ultimaDoCliente?.nomeContato || '',
      ultimaMsg: m.texto || ('['+m.tipo+']'), fromMe: m.fromMe, timestamp: m.timestamp,
      aguardaResposta,
      ultimaMsgClienteTimestamp: g.ultimaDoCliente?.timestamp || null
    };
  }).sort((a,b) => (b.timestamp||0) - (a.timestamp||0));
  res.json({ ok:true, data: lista });
});

// Marca uma conversa como "resolvida" (não precisa mais aparecer como
// aguardando resposta) — até o cliente escrever de novo, aí volta sozinho.
app.post('/api/whatsapp/resolver', async (req, res) => {
  const { instancia, telefone } = req.body || {};
  if (!instancia || !telefone) return res.status(400).json({ ok:false, erro:'instancia e telefone são obrigatórios' });
  const db = await lerDB();
  const msgsDaConversa = (db.whatsapp_mensagens || []).filter(m => m.instancia === instancia && m.telefone === telefone);
  if (msgsDaConversa.length === 0) return res.status(404).json({ ok:false, erro:'conversa não encontrada' });
  const ultimoTs = Math.max(...msgsDaConversa.map(m => m.timestamp || 0));
  if (!db.whatsapp_resolvidos) db.whatsapp_resolvidos = {};
  db.whatsapp_resolvidos[instancia+'|'+telefone] = { ateTimestamp: ultimoTs, resolvidoEm: Date.now() };
  await salvarDB(db);
  res.json({ ok:true });
});

// Histórico completo de UMA conversa (instância + telefone)
app.get('/api/whatsapp/mensagens', async (req, res) => {
  const { instancia, telefone } = req.query;
  if (!instancia || !telefone) return res.status(400).json({ ok:false, erro:'instancia e telefone são obrigatórios' });
  const db = await lerDB();
  const msgs = (db.whatsapp_mensagens || [])
    .filter(m => m.instancia === instancia && m.telefone === telefone)
    .sort((a,b) => (a.timestamp||0) - (b.timestamp||0));
  res.json({ ok:true, data: msgs });
});


// Webhook que recebe mensagens do WhatsApp (formato Evolution API v2)
// Guarda os últimos eventos brutos recebidos (só na memória, não no banco)
// pra dar pra conferir o que está chegando de verdade sem precisar vasculhar
// log do Railway — mesma ideia do /api/bling/situacoes de hoje mais cedo.
const ultimosEventosWhats = [];
app.get('/api/evolution/debug-eventos', (req, res) => {
  res.json({ ok:true, data: ultimosEventosWhats });
});

app.post('/webhook/whatsapp', async (req, res) => {
  res.status(200).json({ ok: true }); // responder rápido, a Evolution não deve reter isso

  // Confere o segredo combinado na URL do webhook (configurado na criação da
  // instância). Sem isso, qualquer um que descobrisse essa URL conseguiria
  // injetar mensagem falsa no sistema.
  if (EVOLUTION_WEBHOOK_SECRET && req.query.secret !== EVOLUTION_WEBHOOK_SECRET) {
    console.warn('⚠️ /webhook/whatsapp recebido com segredo ausente/incorreto — ignorado.');
    return;
  }

  const registro = { recebidoEm: new Date().toISOString(), event: null, instancia: null, resultado: null };
  ultimosEventosWhats.push(registro);
  if (ultimosEventosWhats.length > 30) ultimosEventosWhats.shift();

  try {
    const evento = req.body;
    if (!evento) { registro.resultado = 'corpo vazio'; return; }
    registro.event = evento.event;
    registro.instancia = evento.instance;

    const db = await lerDB();
    if (!db.whatsapp_mensagens) db.whatsapp_mensagens = [];

    // A Evolution API manda event:"messages.upsert" com os dados da mensagem
    // dentro de data (ou data.messages[] dependendo da versão/config).
    if (evento.event !== 'messages.upsert') { registro.resultado = 'ignorado (evento não é messages.upsert)'; return; }
    const listaMsgs = Array.isArray(evento.data?.messages) ? evento.data.messages : [evento.data];
    registro.qtdRecebida = listaMsgs.length;
    registro.qtdSalva = 0;
    registro.motivosPulados = [];

    for (const msg of listaMsgs) {
      if (!msg || !msg.key) { registro.motivosPulados.push('sem key'); continue; }
      // Ignora atualizações de status/reação sem conteúdo de mensagem de verdade
      const m = msg.message || {};
      ...
      const tipo = m.imageMessage ? 'imagem'
                 ...
                 : m.conversation || m.extendedTextMessage ? 'texto'
                 : 'outro';
      if (tipo === 'outro' && !texto) { registro.motivosPulados.push('tipo não reconhecido: ' + Object.keys(m).join(',')); continue; }
      // Guarda a estrutura bruta de áudio no diagnóstico (só quando for áudio)
      // pra dar pra ver os nomes de campo reais antes de construir a busca do
      // arquivo/transcrição, em vez de adivinhar o formato.
      if (tipo === 'audio' && !registro.amostraAudio) registro.amostraAudio = m.audioMessage;

      const telefone = (msg.key.remoteJid || '').replace('@s.whatsapp.net', '').replace('@c.us', '');
      const fromMe = !!msg.key.fromMe;
      const nomeContato = msg.pushName || '';

      // A Evolution API pode reenviar o mesmo evento mais de uma vez (ex: quando
      // confirma entrega/leitura). Sem essa checagem, a mesma mensagem entrava
      // duplicada toda vez que isso acontecesse.
      const idMsg = msg.key.id || '';
      const jaSalva = idMsg && db.whatsapp_mensagens.some(x => x.id === idMsg);
      if (jaSalva) { registro.motivosPulados.push('duplicada (id já salvo): ' + idMsg); continue; }

      db.whatsapp_mensagens.push({
        id: idMsg || Date.now().toString(36),
      ...
      // Limite alto (não é mais só "conversa recente pra revisar atendimento" —
      // agora também serve de histórico pra localizar o que foi dito com um
      // cliente meses depois). Ainda existe um teto pra não crescer pra
      // sempre sem controle, mas bem mais folgado que antes.
      if (db.whatsapp_mensagens.length > 200000) {
        db.whatsapp_mensagens = db.whatsapp_mensagens.slice(-200000);
      }
    ...
    registro.resultado = `ok — ${registro.qtdSalva}/${registro.qtdRecebida} salvas`;
  } catch(e) {
    registro.resultado = 'ERRO: ' + e.message;
...
app.post('/webhook/bling', async (req, res) => {
  ...
  try {
    ...
    for (const evento of eventos) {
      ...
      const det = await httpsGet('api.bling.com.br', `/Api/v3/pedidos/vendas/${idBling}`,
        { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' });
      if (!det.data) continue;
      ...
      // Pedido já importado antes — não duplica, mas checa se a situação virou
      // Cancelado. É esse caminho que dá o cancelamento quase em tempo real
      // (assumindo que o Bling manda webhook de "pedido atualizado" e não só
      // de "pedido criado" — vale confirmar na aba Webhooks do app no Bling).
      if (pedidoExistente(db, numPedido)) {
        const mudou = sincronizarSituacaoCancelamento(db, numPedido, det.data.situacao);
        if (mudou > 0) {
          await salvarDB(db);
          console.log(`Webhook: pedido #${numPedido} cancelado via Bling (${mudou} item(ns))`);
        }
        continue;
      }
      if (pedidoIgnorado(db, numPedido)) continue; // excluído por decisão — não reimportar
...
app.listen(PORT, async () => {
  ...
  setInterval(async () => {
    ...
    try {
      ...
      if (novos > 0 || canceladosAuto > 0) {
        await salvarDB(db);
        console.log(`Auto-sync: ${novos} pedido(s) novo(s), ${canceladosAuto} cancelado(s) via Bling`);
      }
