...
function parsearPedidoBling(p) {
  ...
  return [montarPedido(textoColchao, valor, '')];
}
...
app.get('/api/bling/sync', async (req, res) => {
  ...
  try {
    ...
    const data = await httpsGet('www.bling.com.br',
      `/Api/v3/pedidos/vendas?limite=100&pagina=1&idsSituacoes[]=6&idsSituacoes[]=9&idsSituacoes[]=15&idsSituacoes[]=12&idsSituacoes[]=24&idsSituacoes[]=2&idsSituacoes[]=3&idsSituacoes[]=4&idsSituacoes[]=10${filtroData}`,
      { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' });
...
app.listen(PORT, async () => {
  ...
  setInterval(async () => {
    ...
    try {
      ...
      const db = await lerDB();
      const data = await httpsGet('www.bling.com.br',
        `/Api/v3/pedidos/vendas?limite=100&pagina=1&idsSituacoes[]=6&idsSituacoes[]=9&idsSituacoes[]=15&idsSituacoes[]=12&idsSituacoes[]=24&idsSituacoes[]=2&idsSituacoes[]=3&idsSituacoes[]=4&idsSituacoes[]=10&dataInicial=${dataIni}`,
        { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' });
      if (!data.data) return;
      let novos = 0;
      for (const pedBling of data.data) {
  ...
  }, 3 * 60 * 1000); // a cada 3 minutos (backup do webhook)
});
