...
function parsearPedidoBling(p) {
  ...
  // Buscar endereço em todos os campos possíveis da API v3
  const endT = p.transporte?.enderecoEntrega;
  const endC = p.contato?.endereco;
  // API v3 também pode usar p.enderecoEntrega diretamente
  const endD = p.enderecoEntrega;
  // Escolher o que tiver município/uf
  const endObj = [endT, endD, endC].find(e => e?.municipio || e?.cidade || e?.uf) || endT || endC || endD || {};
  const cidade = (endObj.municipio || endObj.cidade || '').trim();
  const estado = (endObj.uf || endObj.estado || '').trim().toUpperCase();
  // Log para debug (remover depois)
  if (!cidade) console.log('Sem cidade — p.numero:', p.numero, 'transporte:', JSON.stringify(p.transporte)?.slice(0,200), 'contato.end:', JSON.stringify(p.contato?.endereco)?.slice(0,200));
  const endStr = endObj
    ? [endObj.endereco||endObj.logradouro, endObj.numero, endObj.bairro].filter(Boolean).join(', ')
  ...
  // Calcular custos
  const tam = tamSrv(itens);
  const custoColchao = calcCustoSrv(itens, kit, tam);
  let custoAgregados = (/TRAVESSEIRO/i.test(itens) ? 2 : 0) * C_SRV.travesseiro;
  if (/BOX\s*BA[ÚU]/i.test(itens) && C_SRV.base_bau[tam]) custoAgregados += C_SRV.base_bau[tam];
  else if (/BASE\s*BOX/i.test(itens) && C_SRV.base_normal[tam]) custoAgregados += C_SRV.base_normal[tam];
  if (/CABECEIRA\s*P[ÉE]ROLA/i.test(itens) && C_SRV.cab_perola[tam]) custoAgregados += C_SRV.cab_perola[tam];
  custoAgregados = Math.round(custoAgregados);
  const custoFrete = C_SRV.frete[freteiro] || 0;
  const custoTotal = custoColchao + custoAgregados + custoFrete;

  // Taxa pagamento
  const taxaMap = {credito12:0.0599,credito10:0.0499,credito6:0.0369,credito3:0.0249,pix:0};
  const taxa = taxaMap[formaPagto] || 0;
  const taxaValor = Math.round(valor * taxa);
...
app.get('/api/bling/sync', async (req, res) => {
  ...
  try {
    ...
    for (const pedBling of data.data) {
      ...
      // Buscar detalhes só para pedidos novos
      ...
      // Salvar a cada 10 pedidos para não perder em caso de timeout
