...
// DIAGNÓSTICO: testa cada passo e mostra a resposta crua
app.get('/api/wpp/diagnostico/:nome', async (req, res) => {
  ...
  resultado.passo1_gerar_token = {
    secret_usado: WPP_SECRET,
    status: tokenResp.status,
    ok: tokenResp.ok,
    resposta: tokenResp.data
  };
  ...
  if (token) {
    ...
    // 2. Iniciar sessão
    const startResp = await wppRequest('POST', `/api/${nome}/start-session`, { waitQrCode: true }, token);
    resultado.passo2_start_session = {
      status: startResp.status,
      ok: startResp.ok,
      tem_qrcode: !!(startResp.data?.qrcode),
      resposta: typeof startResp.data === 'object' ? Object.keys(startResp.data) : startResp.data
    };
  } else {
    resultado.passo2_start_session = 'PULADO - sem token';
  }
...
app.post('/api/evolution/criar-instancia', async (req, res) => {
  ...
  res.json({ ok: r.ok, data: r.data, token });
});
