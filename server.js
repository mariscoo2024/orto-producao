...
const EVOLUTION_URL = process.env.EVOLUTION_URL || 'https://fabulous-comfort-production-1bf7.up.railway.app';
const EVOLUTION_KEY = process.env.EVOLUTION_KEY || 'ortopremium2026';

// Helper para chamadas à Evolution API
async function evolutionRequest(metodo, caminho, body = null) {
  const url = EVOLUTION_URL + caminho;
  const opts = {
    method: metodo,
    headers: {
...
app.get('/api/evolution/instancias', async (req, res) => {
  const r = await evolutionRequest('GET', '/instance/fetchInstances');
  res.json(r);
});

// Criar nova instância (gera QR Code)
app.post('/api/evolution/criar-instancia', async (req, res) => {
  const { nome } = req.body;
  if (!nome) return res.status(400).json({ erro: 'Nome da instância obrigatório' });
  const r = await evolutionRequest('POST', '/instance/create', {
    instanceName: nome,
    qrcode: true,
    integration: 'WHATSAPP-BAILEYS',
    webhook: {
      url: (process.env.MEU_DOMINIO || 'https://orto-producao-production.up.railway.app') + '/webhook/whatsapp',
...
// Pegar o QR Code de uma instância
app.get('/api/evolution/qrcode/:nome', async (req, res) => {
  const r = await evolutionRequest('GET', `/instance/connect/${req.params.nome}`);
  res.json(r);
});

// Status de uma instância
app.get('/api/evolution/status/:nome', async (req, res) => {
  const r = await evolutionRequest('GET', `/instance/connectionState/${req.params.nome}`);
  res.json(r);
});

// Desconectar/deletar instância
app.delete('/api/evolution/instancia/:nome', async (req, res) => {
  const r = await evolutionRequest('DELETE', `/instance/delete/${req.params.nome}`);
  res.json(r);
});
...
app.post('/webhook/whatsapp', async (req, res) => {
  ...
  try {
    ...
    // Salvar evento no banco para processamento
    const db = await lerDB();
    ...
    if (evento.event === 'messages.upsert') {
      ...
      const tipo = msg.message?.audioMessage ? 'audio'
      ...
      // Limitar a últimas 5000 mensagens para não estourar o banco
      if (db.whatsapp_mensagens.length > 5000) {
        db.whatsapp_mensagens = db.whatsapp_mensagens.slice(-5000);
      }

      // Se mensagem veio de cliente novo (não fromMe), criar lead automático
      if (!fromMe && telefone) {
