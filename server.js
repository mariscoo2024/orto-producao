...
function parsearPedidoBling(p) {
  ...
  const montarPedido = (textoColchao, valorCol, sufixo) => {
    ...
    // Frete: ARW distingue cidade próxima (R$70) vs distante (R$90)
    const cidadesArwLong = ['almirante tamandare','campo largo','campo magro'];
    let chaveFrete = freteiro;
    if (freteiro === 'ARW' && cidadesArwLong.some(x => c.includes(x))) {
      chaveFrete = 'ARW_LONG';
    }
    const custoFreteC = colchoesItens.length > 1 ? 0 : (C_SRV.frete[chaveFrete] || 0); // frete só no primeiro se múltiplos
