...
async function garantirSeedConciliacao(db) {
  if (db.conciliacaoBanco && db.conciliacaoBanco.length > 0) return db;
  db.conciliacaoBanco = JSON.parse(JSON.stringify(SEED_CONCILIACAO_BANCO));
  await salvarDB(db);
  console.log(`✅ Seed de conciliação bancária aplicado: ${db.conciliacaoBanco.length} nomes mapeados`);
  return db;
}
...
{"data":"AAAA-MM-DD","descricao":"string curta","valor":0.00,"categoriaSugerida":"uma das fixas ou 'avulso'","tagCurta":"só se for avulso — nome curto da categoria, ex Combustível","provavelmentePessoal":false,"confianca":"alta|media|baixa","observacao":""}
