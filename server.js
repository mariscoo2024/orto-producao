...
app.post('/api/extrato/analisar', async (req, res) => {
  try {
    ...
    if (!texto || !texto.trim()) return res.status(400).json({ ok:false, erro:'texto do extrato é obrigatório' });
    if (!ANTHROPIC_API_KEY) return res.status(500).json({ ok:false, erro:'ANTHROPIC_API_KEY não configurada no servidor' });
    ...
    const prompt = `Você está lendo um extrato bancário semanal (PagBank) de uma fábrica de colchões (Orto Premium), pra separar despesas pequenas do dia a dia por categoria, já que hoje isso é feito só no fim do mês, tudo misturado.
