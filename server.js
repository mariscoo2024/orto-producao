...
// Servir arquivos estáticos (css, js, imagens se houver)
app.use(express.static(path.join(__dirname)));

// Rota raiz e catch-all — sempre serve index.html
app.get('/', (req, res) => res.sendFile(path.join(__dirname, 'index.html')));
app.get('*', (req, res) => {
  // Não interceptar rotas de API
  if (req.path.startsWith('/api/') || req.path.startsWith('/auth/')) return res.status(404).json({erro:'não encontrado'});
